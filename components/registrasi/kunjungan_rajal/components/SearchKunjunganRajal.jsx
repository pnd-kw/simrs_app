...
import { translateDay } from "@/utils/schedule";
...
const mappedData = (data) =>
  data.map((item) => ({
    ...
    jadwal_praktek: (
      <div>{`${translateDay(item.doctor_schedule_day)}, ${
        item.doctor_schedule_time_start
      } - ${item.doctor_schedule_time_finish}`}</div>
    ),
    ...
    occupation: item.rm_occupation_name,
    ...
    poli_layanan: item.poli_layanan.value,
...
const SearchKunjunganRajal = () => {
  ...
  useEffect(() => {
    if (isSuccessSubmitRajalRs && !isFetching && data?.items?.length > 0) {
      setRajalRsData(data.items[0]);
      setIsSuccessSubmitRajalRs(false);
    }
  }, [isSuccessSubmitRajalRs, data]);
  ...
  useEffect(() => {
    if (
      ...
      const found = data.items.find((item) => item.id === selectedId);
      if (found) setRajalRsData(found);
      setIsSuccessUpdateRajalRs(false);
    }
  }, [isSuccessUpdateRajalRs, data]);
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 px-2">
            ...
            <div className="grid grid-rows-3 max-h-[12vh]">
              ...
              <div className="flex items-center">
                <div className="w-1/3">
                  <label htmlFor="poliklinik_id" className="label-style">
                    poliklinik
                  </label>
                </div>
                <div className="relative w-2/3">
...
export default SearchKunjunganRajal;
