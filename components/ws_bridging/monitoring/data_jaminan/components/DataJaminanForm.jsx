...
import { useDataJaminanStore } from "@/stores/ws_bridging/useWsBridgingStore";
...
const DataJaminanForm = () => {
  ...
  const formattedData = dataJaminan.map((item) => ({
    no_rm: item.namaPeserta || "-",
    nama_pasien: item.namaPeserta || "-",
    no_kartu: item.noKartu || "-",
    no_sep: item.noSep || "-",
    poli: item.poli || "-",
    tgl_pulang: item.tglPlgSep || "-",
    tgl_sep: item.tglSep || "-",
  }));
