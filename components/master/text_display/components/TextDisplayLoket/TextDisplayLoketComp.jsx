...
import {
  getTextDisplayLoketCheckboxes,
  getTextDisplayLoketFields,
  textDisplayLoketSchema,
...
const TextDisplayLoketComp = () => {
  const {
    ...
    resolver: zodResolver(textDisplayLoketSchema),
    defaultValues: {
      judul: "",
      text: "",
      duration: "",
  ...
  const [searchTextDisplayLoket, setSearchTextDisplayLoket] = useState("");
  ...
  const fetchData = async (query = "per_page=10") => {
    try {
      setIsLoading(true);
      const data = await getTextDisplayLoket(query);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toastWithProgress({
        title: "Berhasil",
        description: "Data Text Display Loket berhasil disimpan.",
        type: "success",
      });

      setOpenModal(false);
    } catch (err) {
      console.error("Gagal simpan Text Display Loket:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Text Display Loket",
        type: "error",
      });
    }
  };
  ...
  const fields = getTextDisplayLoketFields();
  const checkboxes = getTextDisplayLoketCheckboxes();
  ...
  const filteredTextDisplayLoket = formattedData.filter((item) => {
    const searchLower = searchTextDisplayLoket.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField
                value={searchTextDisplayLoket}
                onChange={setSearchTextDisplayLoket}
              />
            </div>
            <Button
              onClick={() => {
          ...
          <Table
            data={
              searchTextDisplayLoket ? filteredTextDisplayLoket : formattedData
            }
...
export default TextDisplayLoketComp;
