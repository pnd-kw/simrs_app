...
import {
  getGolKelasAplicareFields,
  golKelasAplicareSchema,
} from "./GolKelasAplicareFields";
import SearchField from "@/utils/table/searchField";
import Spinner from "@/utils/spinner";

const GolKelasAplicare = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    resolver: zodResolver(golKelasAplicareSchema),
    defaultValues: {
      kodebridging: "",
      name: "",
      remark: "",
  ...
  const [searchGolKelasAplicare, setSearchGolKelasAplicare] = useState("");

  const fields = getGolKelasAplicareFields();

  const mappedData = (data) =>
    data.map((item) => ({
      id: item.id,
      ruang_id: item.ruang_id?.id,
      kode: item.kodebridging,
      nama: item.name,
      remark: item.remark || "-",
  ...
  const fetchData = async (query = "per_page=10") => {
    try {
      setIsLoading(true);
      const data = await getKelasRuangAplicare(query);
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedRow.id) {
        await updateKelasRuangAplicare({
          ...data,
          id: selectedRow.id,
        });
      } else {
        await postKelasRuangAplicare(data);
      }
      fetchData();
      reset();
      setSelectedRow({});
      ...
      toastWithProgress({
        title: "Berhasil",
        description: "Data Kelas Gol Kelas Aplicare berhasil disimpan.",
        type: "success",
      });

      setOpenModal(false);
    } catch (err) {
      console.error("Gagal simpan Kelas Gol Kelas Aplicare:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Kelas Gol Kelas Aplicare",
        type: "error",
      });
    }
  };

  const filteredGolKelasAplicare = formattedData.filter((item) => {
    const searchLower = searchGolKelasAplicare.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto w-full min-h-screen">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField
                value={searchGolKelasAplicare}
                onChange={setSearchGolKelasAplicare}
              />
            </div>
            <Button
              onClick={() => {
          ...
          <Table
            data={
              searchGolKelasAplicare ? filteredGolKelasAplicare : formattedData
            }
          ...
          <FormModalZod
            ...
            title={
              selectedRow?.id
                ? "Edit Gol Kelas Aplicare"
                : "Tambah Gol Kelas Aplicare"
            }
            fields={fields}
            errors={errors}
            control={control}
...
export default GolKelasAplicare;
