...
import { getKamarFields, kamarSchema } from "./KamarFields";
...
const Kamar = () => {
  ...
  const fields = getKamarFields(ruangan);
  ...
  const mappedData = (data) =>
    data.map((item) => ({
      ...
      nama: item.name,
      gudang_farmasi: item.id_gudang_farmasi?.id || "-",
      fasilitas: item.id_fasilitas?.id || "-",
      remark: item.remark || "-",
  ...
  const fetchData = async (query = "per_page=10") => {
    try {
      setIsLoading(true);
      const data = await getKamar(query);
  ...
  const filteredKamar = formattedData.filter((item) => {
    const searchLower = searchKamar.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto w-full min-h-screen">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField value={searchKamar} onChange={setSearchKamar} />
            </div>
            <Button
              onClick={() => {
                setSelectedRow({});
          ...
          <Table
            data={searchKamar ? filteredKamar : formattedData}
          ...
          <FormModalZod
            ...
            title={selectedRow?.id ? "Edit Kamar" : "Tambah Kamar"}
            fields={fields}
            errors={errors}
            control={control}
            submitLabel="Simpan"
