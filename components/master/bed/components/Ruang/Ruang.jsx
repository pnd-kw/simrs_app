...
import { getRuangFields, ruangSchema } from "../RuangFields";
...
const Ruang = () => {
  ...
  const fetchData = async (query = "per_page=10") => {
    try {
      setIsLoading(true);
      const data = await getRuang(query);
  ...
  const filteredRuang = formattedData.filter((item) => {
    const searchLower = searchRuang.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto w-full min-h-screen">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField value={searchRuang} onChange={setSearchRuang} />
            </div>
            <Button
              onClick={() => {
                setSelectedRow({});
          ...
          <Table
            data={searchRuang ? filteredRuang : formattedData}
