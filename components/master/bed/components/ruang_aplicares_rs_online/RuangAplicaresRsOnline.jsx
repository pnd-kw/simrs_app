...
import {
  getRuangAplicaresRsOnlineFields,
  ruangAplicaresRsOnlineSchema,
} from "./RuangAplicaresRsOnlineFields";

const RuangAplicaresRsOnline = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    resolver: zodResolver(ruangAplicaresRsOnlineSchema),
    defaultValues: {
      kode: "",
      name: "",
      remark: "",
  ...
  const [searchRuangAplicareRsOnline, setSearchRuangAplicareRsOnline] =
    useState("");

  const fields = getRuangAplicaresRsOnlineFields();
  ...
  const mappedData = (data) =>
    data.map((item) => ({
      ...
      kode: item.kode,
      nama: item.name,
      kode_sirs_ol: item.kode_sirsonline || "-",
      remark: item.remark || "-",
  ...
  const fetchData = async (query = "per_page=10") => {
    try {
      setIsLoading(true);
      const data = await getRuangAplicare(query);
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedRow.id) {
        await updateRuangAplicare({
          ...data,
          id: selectedRow.id,
        });
      } else {
        await postRuangAplicare(data);
      }
      fetchData();
      reset();
      setSelectedRow({});
      ...
      toastWithProgress({
        title: "Berhasil",
        description: "Data Ruang Aplicare berhasil disimpan.",
        type: "success",
      });

      setOpenModal(false);
    } catch (err) {
      console.error("Gagal simpan Ruang Aplicare:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Ruang Aplicare",
        type: "error",
      });
    }
  };

  const filteredRuangAplicareRsOnline = formattedData.filter((item) => {
    const searchLower = searchRuangAplicareRsOnline.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto w-full min-h-screen">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField
                value={searchRuangAplicareRsOnline}
                onChange={setSearchRuangAplicareRsOnline}
              />
            </div>
            <Button
              onClick={() => {
          ...
          <Table
            data={
              searchRuangAplicareRsOnline
                ? filteredRuangAplicareRsOnline
                : formattedData
            }
            ...
            customWidths={{
              nama: "min-w-[20rem]",
              kode: "min-w-[10rem]",
              kode_sirs_ol: "min-w-[15rem]",
              remark: "min-w-[15rem]",
              inserted_time: "min-w-[13rem]",
          ...
          <FormModalZod
            ...
            title={
              selectedRow?.id ? "Edit Ruang Aplicare" : "Tambah Ruang Aplicare"
            }
            fields={fields}
            errors={errors}
            control={control}
...
export default RuangAplicaresRsOnline;
