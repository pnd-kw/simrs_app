...
} from "./KelasKunjunganFields";

const KelasKunjunganComp = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const fetchData = async (queryString) => {
    try {
      setIsLoading(true);
      const data = await getKelasKunjungan(queryString);
  ...
  const fields = getKelasKunjunganFields(listTarif);
  const checkboxes = getKelasKunjunganCheckboxes();

  useEffect(() => {
    if (selectedRow?.id) {
      reset({
        name: selectedRow.nama || "",
        tarif_id: selectedRow.tarif_id?.id || undefined,
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedRow.id) {
        await updateKelasKunjungan({
          ...data,
          id: selectedRow.id,
        });
      } else {
        await postKelasKunjungan(data);
      }
      fetchData();
      reset();
      ...
      toastWithProgress({
        title: "Berhasil",
        description: "Data Kelas Kunjungan berhasil disimpan.",
        type: "success",
      });

      setOpenModal(false);
    } catch (err) {
      console.error("Gagal simpan Kelas Kunjungan:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Kelas Kunjungan",
        type: "error",
      });
    }
  };
  ...
  useEffect(() => {
    const fetchTarif = async () => {
      try {
        const res = await getTarif();
        const options = res.data.map((item) => ({
          label: `${item.name}`,
          value: item.id,
        }));

        setListTarif(options);
      } catch (err) {
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            ...
            <Button
              onClick={() => {
                setSelectedRow({});
                reset({ name: "", tarif_id: null, remark: "", isactive: true });
                setOpenModal(true);
              }}
...
export default KelasKunjunganComp;
