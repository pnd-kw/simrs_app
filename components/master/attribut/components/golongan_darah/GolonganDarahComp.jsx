...
import {
  getGolonganDarahFields,
  golonganDarahSchema,
} from "./GolonganDarahFields";
import Spinner from "@/utils/spinner";
import SearchField from "@/utils/table/searchField";

const GolonganDarahComp = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const [searchGolonganDarah, setSearchGolonganDarah] = useState("");

  const fields = getGolonganDarahFields();
  ...
  const fetchData = async (query = "") => {
    try {
      setIsLoading(true);
      const data = await getGolonganDarah(query);
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedRow.id) {
        await updateGolonganDarah({
          ...data,
          id: selectedRow.id,
        });
      } else {
        await postGolonganDarah(data);
      }
      fetchData();
      reset();
      setSelectedRow({});
      ...
      toastWithProgress({
        title: "Berhasil",
        description: "Data Golongan Darah berhasil disimpan.",
        type: "success",
      });

      setOpenModal(false);
    } catch (err) {
      console.error("Gagal simpan Golongan Darah:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Golongan Darah",
        type: "error",
      });
    }
  };
  ...
  const filteredGolonganDarah = formattedData.filter((item) => {
    const searchLower = searchGolonganDarah.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField
                value={searchGolonganDarah}
                onChange={setSearchGolonganDarah}
              />
            </div>
            <Button
              onClick={() => {
          ...
          <Table
            data={searchGolonganDarah ? filteredGolonganDarah : formattedData}
          ...
          <FormModalZod
            ...
            title={
              selectedRow?.id ? "Edit Golongan Darah" : "Tambah Golongan Darah"
            }
            fields={fields}
            errors={errors}
            control={control}
...
export default GolonganDarahComp;
