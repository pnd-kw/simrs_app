...
import { agamaSchema, getAgamaFields } from "./AgamaFields";
...
const AgamaComp = () => {
  ...
  const fetchData = async (query = "") => {
    try {
      setIsLoading(true);
      const data = await getAgama(query);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.error("Gagal simpan agama:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan agama",
        type: "error",
      });
    }
  };
  ...
  const filteredAgama = formattedData.filter((item) => {
    const searchLower = searchAgama.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField value={searchAgama} onChange={setSearchAgama} />
            </div>
            <Button
          ...
          <Table
            data={searchAgama ? filteredAgama : formattedData}
          ...
          <FormModalZod
            ...
            title={selectedRow?.id ? "Edit Agama" : "Tambah Agama"}
            fields={fields}
            errors={errors}
            control={control}
            submitLabel="Simpan"
