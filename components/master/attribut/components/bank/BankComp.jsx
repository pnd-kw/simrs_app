...
import { bankSchema, getBankFields } from "./BankFields";
...
const BankComp = () => {
  ...
  const fetchData = async (query = "") => {
    try {
      setIsLoading(true);
      const data = await getBank(query);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.error("Gagal simpan Bank:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Bank",
        type: "error",
      });
    }
  };
  ...
  const filteredBank = formattedData.filter((item) => {
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField value={searchBank} onChange={setSearchBank} />
            </div>
            <Button
              onClick={() => {
                setOpenModal(true);
          ...
          <Table
            data={searchBank ? filteredBank : formattedData}
          ...
          <FormModalZod
            ...
            title={selectedRow?.id ? "Edit Bank" : "Tambah Bank"}
            fields={fields}
            errors={errors}
            control={control}
            submitLabel="Simpan"
