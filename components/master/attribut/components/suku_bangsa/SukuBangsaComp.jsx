...
import { getSukuBangsaFields, sukuBangsaSchema } from "./SukuBangsaFields";
import Spinner from "@/utils/spinner";

const SukuBangsaComp = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    resolver: zodResolver(sukuBangsaSchema),
    defaultValues: {
      name: "",
    },
  });
  ...
  const [searchSukuBangsa, setSearchSukuBangsa] = useState("");

  const fields = getSukuBangsaFields();
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedRow.id) {
        await updateSukuBangsa({
          ...data,
          id: selectedRow.id,
        });
      } else {
        await postSukuBangsa(data);
      }
      fetchData();
      reset();
      setSelectedRow({});
      ...
      toastWithProgress({
        title: "Berhasil",
        description: "Data Suku berhasil disimpan.",
        type: "success",
      });

      setOpenModal(false);
    } catch (err) {
      console.error("Gagal simpan Suku:", err);
      toastWithProgress({
        title: "Gagal",
        description: "Gagal simpan Suku",
        type: "error",
      });
    }
  };
  ...
  const filteredSukuBangsa = formattedData.filter((item) => {
    const searchLower = searchSukuBangsa.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField
                value={searchSukuBangsa}
                onChange={setSearchSukuBangsa}
              />
            </div>
            <Button
              onClick={() => {
          ...
          <Table
            data={searchSukuBangsa ? filteredSukuBangsa : formattedData}
          ...
          <FormModalZod
            ...
            title={selectedRow?.id ? "Edit Suku" : "Tambah Suku"}
            fields={fields}
            errors={errors}
            control={control}
            submitLabel="Simpan"
