...
import {
  getMasterJenisHargaFields,
  MasterJenisHargaSchema,
} from "./MasterJenisHargaFields";
import SearchField from "@/utils/table/searchField";
import { Button } from "@headlessui/react";
import Spinner from "@/utils/spinner";

const MasterJenisHargaComp = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    resolver: zodResolver(MasterJenisHargaSchema),
    defaultValues: {
      name: "",
      code: "",
    },
  ...
  const [searchJenisHarga, setSearchJenisHarga] = useState("");
  ...
  const fetchData = async (query = "") => {
    try {
      setIsLoading(true);
      const data = await getMasterJenisHarga(query);
  ...
  const fields = getMasterJenisHargaFields();
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedRow.id) {
        await updateMasterJenisHarga({
          ...data,
          id: selectedRow.id,
        });
      } else {
        await postMasterJenisHarga(data);
      }
      fetchData();
      reset();
  ...
  const filteredJenisHarga = formattedData.filter((item) => {
    const searchLower = searchJenisHarga.toLowerCase();
      ...
      {isLoading ? (
        ...
        <div className="p-2 mx-auto">
          <div className="flex justify-end p-2 mb-2 gap-2">
            <div>
              <SearchField
                value={searchJenisHarga}
                onChange={setSearchJenisHarga}
              />
            </div>
            <Button
              onClick={() => {
          ...
          <Table
            data={searchJenisHarga ? filteredJenisHarga : formattedData}
            ...
            customWidths={{
              ...
              updated_time: "min-w-[19.5rem]",
              updated_by: "min-w-[20rem]",
            }}
...
export default MasterJenisHargaComp;
